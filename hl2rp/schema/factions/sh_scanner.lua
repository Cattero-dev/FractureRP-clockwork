...
local FACTION = Clockwork.faction:New("Сканер");
...
function FACTION:GetName(player, character)
	return "C17.MPF-SCN:"..Clockwork.kernel:ZeroNumberToDigits(math.random(1, 999), 3);
end;
...
function FACTION:OnTransferred(player, faction, name)
	if (faction.name == FACTION_OTA or faction.name == FACTION_MPF) then
		if (name) then
			Clockwork.player:SetName(player, string.gsub(player:QueryCharacter("name"), ".+(%d%d%d)", "MPF-SCN.%1"), true);
		else
