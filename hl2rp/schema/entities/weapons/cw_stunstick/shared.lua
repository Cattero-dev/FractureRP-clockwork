			...
			if (IsValid(trace.Entity)) then
				...
				if (trace.Entity:IsPlayer()) then
					...
					if (trace.Entity:Health() > 10) then
						trace.Entity:TakeDamageInfo(Clockwork.kernel:FakeDamageInfo(1, self, self.Owner, trace.HitPos, DMG_CLUB, 2));
					end;
				...
				elseif (IsValid(trace.Entity:GetPhysicsObject())) then
					...
					if (!player or player:Health() > 10) then
						if (!player) then
							trace.Entity:TakeDamageInfo(Clockwork.kernel:FakeDamageInfo(5, self, self.Owner, trace.HitPos, DMG_CLUB, 2));
						else
							trace.Entity:TakeDamageInfo(Clockwork.kernel:FakeDamageInfo(1, self, self.Owner, trace.HitPos, DMG_CLUB, 2));
						end;
					end;
